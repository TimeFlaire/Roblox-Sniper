            ...
            while True:
                ...
                if not json_response["purchased"]:
                       ...
                       if self.webhookEnabled:
                            embed_data = {
                                "title": "New Item Purchased with Timeflaire Sniper",
                                "url": f"https://www.roblox.com/catalog/{item_id}/Timeflaire-Sniper",
                                "color": 65280,
                                "author": {
                                    "name": "Purchased limited successfully!"
                                },
                                "footer": {
                                "text": "Timeflaire's Sniper"
                                }
                            }
      ...
      while True:
       try:
        async with aiohttp.ClientSession() as session:
            async with session.get("https://catalog.roblox.com/v1/search/items/detailsCategory=1&CurrencyType=3&pxMin=0&pxMax=0&salesTypeFilter=2&SortType=4", ssl = False) as response:
                self.checks += 1       
                if response.status == 429:
